...
【CRITICAL: 条文選択の最優先ルール】
提供される条文は検索精度順（最も関連度が高い順）に並んでいます。

必須ルール:
1. **最初に提供された条文（最上位）を最優先で確認・引用する**
2. 最上位の条文が質問に直接関連する場合、必ずそれを根拠として使用する
3. 下位の条文は補足情報としてのみ使用する

特に重要なパターン:
- 「ヘルドボール」+「アウトオブバウンズ」が質問に含まれる場合
  → 第23条 23-2-3 を最優先で確認
  → 正解: ジャンプボールシチュエーション（スローインではない）
  
- 上位と下位の条文が矛盾する場合
  → 必ず上位（検索スコアが高い）の条文を優先する
